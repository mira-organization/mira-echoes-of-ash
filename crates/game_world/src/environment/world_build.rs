...
use game_core::loading::{EffectSceneAssets, LoadedAssets, WaitingForAreaAssets};
use game_core::states::{AppState, AssetLoadState};
use game_core::world::environment::{CurrentAreaScenes, CurrentEnvironment, EnvironmentScene};
use game_core::world::light::{LightData, LightType};
use std::f32::consts::PI;

pub struct WorldBuildHandler;

impl Plugin for WorldBuildHandler {
    #[coverage(off)]
    fn build(&self, app: &mut App) {
        app.add_systems(OnEnter(AppState::AssetsLoad(AssetLoadState::EnvPreLoad)), (pre_load_area, pre_load_gltf_assets));
        app.add_systems(Update, process_loaded_area.run_if(in_state(AppState::AssetsLoad(AssetLoadState::EnvPreLoad))));
        app.add_systems(Update, load_active_area_lights.run_if(in_state(AppState::AssetsLoad(AssetLoadState::EnvLoad))));
        app.add_systems(OnEnter(AppState::AssetsLoad(AssetLoadState::EnvLoad)), load_active_area);
    }
}
...
pub fn process_loaded_area(mut commands: Commands,
    ...
    if let Some(waiting) = waiting {
        if let Some(gltf) = gltf_assets.get(&waiting.0) {
            ...
            debug!("Found [{:?}] scenes", found_scenes);
            ...
            next_state.set(AppState::AssetsLoad(AssetLoadState::EnvLoad));
            debug!("Finished loading environments");
        }
    }
}
