...
impl Plugin for PingChannel {
    #[coverage(off)]
    fn build(&self, app: &mut App) {
        app.insert_resource(PingTimer(Timer::from_seconds(2.5, TimerMode::Repeating)));
        app.add_systems(OnEnter(AppState::Preload), setup_socket);
        app.add_systems(Update, (send_ping, receive_pong).after(setup_socket));
    }
}
...
#[coverage(off)]
fn setup_socket(mut ping_data: ResMut<PingResponse>) {
    ...
    debug!("Created UDP socket for ping requests!");
}
...
#[coverage(off)]
fn send_ping(time: Res<Time>, mut timer: ResMut<PingTimer>, mut ping_data: ResMut<PingResponse>) {
...
#[coverage(off)]
fn receive_pong(mut ping_data: ResMut<PingResponse>) {
    if let Some(socket) = &ping_data.socket {
        let mut buf = [0u8; 9];
