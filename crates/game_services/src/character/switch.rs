...
use game_core::entities::character::{ChangeCharacter, Character, CharacterPartyInfo, CurrentWorldCharacter};
use game_core::entities::player::{WorldPlayer, WorldPlayerBundle};
use game_core::global_resources::GlobalEntities;
use game_core::key_converter::convert;
use game_core::loading::LoadedAssets;
use game_core::states::is_state_in_game;

pub struct CharacterSwitchLogic;

impl Plugin for CharacterSwitchLogic {
    #[coverage(off)]
    fn build(&self, app: &mut App) {
        app.add_systems(Update, trigger_switch_character.run_if(is_state_in_game));
...
fn trigger_switch_character(
    ...
    let key_map: HashMap<usize, KeyCode> = vec![
        (1, convert(&game_config.input.character_01).expect("Failed to convert key 1")),
        (2, convert(&game_config.input.character_02).expect("Failed to convert key 2")),
        (3, convert(&game_config.input.character_03).expect("Failed to convert key 3")),
        (4, convert(&game_config.input.character_04).expect("Failed to convert key 4")),
    ].into_iter().collect();
...
fn switch_character(
    ...
    if change_character.0 {
        ...
        for raw_character in global_entities.0.iter() {
        ...
        if let Some(data) = json_character {
            ...
            if let Some(mut character) = character {
                ...
                for (key, handles) in assets.entities.clone() {
